...
An `interaction` island must render useful fallback children on the server. The fallback gives pointer, focus, and keyboard interaction a reachable boundary before the Vue module exists:

```vue
<VueIsland name="AccountMenu" trigger="interaction">
  <button type="button">Open account menu</button>
</VueIsland>
```

A `manual` enhancement or island stays dormant until application browser code explicitly activates its element or selector through `window.__RESUX_ACTIVATE_DEFERRED__(targetOrSelector)`. That activation loads the runtime if necessary and then activates the selected deferred target.

Once the full runtime is present, Resux still honors scheduled triggers instead of mounting every deferred island indiscriminately. In-flight island imports are cancellation-aware, so route replacement cannot mount an island into a detached subtree.
