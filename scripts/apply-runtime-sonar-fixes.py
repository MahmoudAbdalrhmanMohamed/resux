...
def replace_exact(source: str, replacement: str, expected: int, label: str) -> None:
    global text
...
replace_exact(
  ...
  callbacks.add(run);''',
    1,
    "generated visible callback registry",
)
...
''',
    1,
    "typed enhancement bound helper",
)
replace_exact(
...
''',
    1,
    "generated enhancement bound helper",
)
...
replace_exact(
    ...
    2,
    "element bound cleanup",
)

typed_start = 'async function activateDeclaredClientEnhancements(root: ParentNode = document): Promise<void> {'
typed_end = '\nfunction uniqueArray<T>(items: T[]): T[] {'
generated_start = 'async function activateDeclaredClientEnhancements(root = document) {'
generated_end = '\nfunction uniqueArray(items) {'

typed_replacement = '''interface DeclaredClientEnhancement {
  name: string;
...
generated_replacement = '''function collectDeclaredClientEnhancementElements(root) {
  const selector = "[data-resux-enhancement], [use-client-enhancement]";
  const elements = [...root.querySelectorAll(selector)];
...
def replace_until(start_marker: str, end_marker: str, replacement: str, label: str) -> None:
    global text
...
replace_until(typed_start, typed_end, typed_replacement, "typed activation refactor")
replace_until(generated_start, generated_end, generated_replacement, "generated activation refactor")

for needle in (
    "[...callbacks]",
    "[...resuxVisibleEnhancementCallbacks]",
    'removeAttribute("data-rx-enhancement-bound")',
):
    if needle in text:
        raise SystemExit(f"remaining Sonar pattern: {needle}")

runtime_path.write_text(text, encoding="utf8")

Path(".github/workflows/runtime-sonar-inspect.yml").unlink(missing_ok=True)
Path("scripts/apply-runtime-sonar-fixes.py").unlink(missing_ok=True)
...
jobs:
  checks:
    name: Build, Test, and Pack Check
    ...
    steps:
      ...
      - name: Verify Rolldown Linux native binding
        if: runner.os == 'Linux'
        run: node -e "require.resolve('@rolldown/binding-linux-x64-gnu')"
      ...
      - name: Package check
        run: npm run pack:check --if-present
''', encoding="utf8")
