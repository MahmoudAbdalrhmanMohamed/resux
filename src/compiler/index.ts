...
import * as internalCompiler from "./internal.js";
import {
  normalizeResuxDirectiveSyntax,
  normalizeResuxSfcSource,
  rethrowWithResuxDirectiveBrand,
} from "./directives.js";
import type { BuildOptions, BuildResult, CompiledComponent } from "./internal.js";
  ...
  return enqueueBuild(async () => {
    try {
      return await withResuxDirectiveFileReads(() => (
        internalCompiler.buildProject(appRoot, outDir, options)
      ));
    } catch (error) {
      return rethrowWithResuxDirectiveBrand(error);
    }
